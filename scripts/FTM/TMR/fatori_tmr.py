...
# =============================================================
#
# This script selects registers from YAML descriptions for
...
# provided/random seed, and yaml constraints. 
# It can handle three types of YAML input files:


# ----------------------------------------------------------------
# 1) Register List YAML (--yaml) [wrapped_registers.yaml]
# * Mandatory
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
# 2) Disable Flags YAML (--dis_flags) [dis_flags.yaml]
# * Optional
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
# 3) File Enable YAML (--file_enable) [forced_enabled_files.yaml]
# * Optional
# ----------------------------------------------------------------
...
def flatten_registers(data, dis_flags=None, verbose=False):
    """Flatten YAML register definitions into dict {file: [reg1, reg2,...]}."""
...
# =============================================================================
# Function: fatori_tmr_exact
# Purpose : Randomly enable registers to match exact percentage
# =============================================================================
def fatori_tmr_exact(registers_by_file, percentage, verbose=False):
...
# Purpose : Handle forced-enable YAML
...
# =============================================================================
def apply_forced_files(registers_by_file, forced_files_yaml=None, include_forced=False, percentage=50, verbose=False):
    """Pre-process forced-enabled files and calculate remaining registers to randomly select.
    
    Supports two formats in forced_files_yaml:
    ...
    # Load the forced-enable YAML if provided and file exists
    if forced_files_yaml:
        if os.path.isfile(forced_files_yaml):
            with open(forced_files_yaml, 'r') as f:
                forced_entries = yaml.safe_load(f)
            ...
            if verbose:
                print(f"[INFO] No forced-enable file found at {forced_files_yaml}, continuing without forced registers.")
...
# =============================================================================
def write_svh_file(output_file, tmr_selection, yaml_file, folder, percentage, seed, registers_by_file):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"// Source YAML   : {yaml_file}\n")
...
# =============================================================================
# Function: main
# Purpose : Parse arguments, read YAMLs, generate TMR selections, write SVH
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="TMR register selector")
    parser.add_argument('--yaml', required=True, help="Input YAML file with register list")
    ...
    parser.add_argument('--file_enable', help="YAML list of files to forcibly enable")
    parser.add_argument('--file_enable_include', action='store_true', help="Include forced files in percentage count")
    parser.add_argument('--dis_flags', help="YAML file with disable flags")
    args = parser.parse_args()

    # Enforce YAML-only by rejecting .yaml paths (to avoid dual support)
    for path_arg, label in [(args.yaml, "--yaml"), (args.dis_flags, "--dis_flags"), (args.file_enable, "--file_enable")]:
        if path_arg and isinstance(path_arg, str) and path_arg.lower().endswith(".yaml"):
            raise SystemExit(f"[ERROR] {label} now expects a YAML file. Please convert '{path_arg}' to .yml/.yaml.")
    ...
    random.seed(seed)
    print(f"[INFO] Starting TMR selection with YAML={args.yaml} Seed={seed}")
    ...
    # ------------------------
    registers_by_file = flatten_registers(yaml_data, dis_flags=dis_flags, verbose=args.verbose)
    ...
    tmr_selection, remaining_regs, num_enabled = apply_forced_files(
        registers_by_file,
        forced_files_yaml=forced_file_arg,
    ...
    write_svh_file(args.out, tmr_selection, args.yaml, folder, args.percentage, seed, registers_by_file)
